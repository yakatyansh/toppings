...
interface PlaylistInfo {
  ...
  const addPlaylistRuntime = async(playlistID: string): Promise<void> => {
    const playlistbar = document.querySelector('.metadata-action-bar') as HTMLDivElement
    ...
    if (MetadataToppings === null) {
      ...
      if (playlistbar.lastChild !== null) {
        playlistbar.insertBefore(
          MetadataToppings,
          playlistbar.lastChild.previousSibling
        )
      }
    } else {
      if (playlistID === 'WL' || playlistID === 'LL') {
